 ...
 */
export const usePayOSPaymentInfo = (
  orderCode: number,
  enabled: boolean = false
) => {
  return useQuery({
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["payment", data.data?.paymentId],
      });
    },
  });
};
...
export const useCancelPayOSPayment = () => {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ["payments"] });
      queryClient.invalidateQueries({
        queryKey: ["payment-order-code", variables.orderCode],
      });
      queryClient.invalidateQueries({
        queryKey: ["payos-info", variables.orderCode],
      });
    },
  });
};

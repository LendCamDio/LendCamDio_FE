...
const PaymentReturnPage: React.FC = () => {
    ...
    useEffect(() => {
        const verifyPayment = async () => {
            ...
            console.log('Payment Return Params:', { code, id, cancel, payosStatus, orderCode });
            ...
            if (code === '00' && payosStatus === 'PAID' && orderCode) {
                try {
                    ...
                    const response = await axios.get(`/api/payments/orders/payos/${orderCode}`);

                    if (response.data && response.data.data && response.data.data.paymentInfo) {
            ...
            <Card className="w-full max-w-md">
                <CardHeader className="text-center">
                    <div className="flex justify-center mb-4">
                        {status === 'loading' && <Loader2 className="w-16 h-16 animate-spin text-blue-600" />}
                        {status === 'success' && <CheckCircle className="w-16 h-16 text-green-600" />}
                        {status === 'failed' && <XCircle className="w-16 h-16 text-red-600" />}
                    </div>
                    <CardTitle className={status === 'success' ? 'text-green-600' : status === 'failed' ? 'text-red-600' : 'text-gray-700'}>
